...
/**
 * Load Telegram credentials from Supabase Secrets
 */
async function loadTelegramCredentials() {
  try {
    ...
    if (!supabaseUrl || !supabaseKey) {
      console.warn('⚠️ SUPABASE_URL or SUPABASE_ANON_KEY not set, Telegram credentials will use environment variables');
      return;
    }

    const response = await fetch(`${supabaseUrl}/rest/v1/rpc/get_telegram_config`, {
      method: 'POST',
      headers: {
        'apikey': supabaseKey,
    ...
    if (response.ok) {
      ...
      if (data && data.telegram_bot_token && data.telegram_chat_id) {
        notificationService.setTelegramCredentials(
          data.telegram_bot_token,
          data.telegram_chat_id
        );
        console.log('✅ Telegram credentials loaded from Supabase Secrets');
      }
    } else {
      console.warn('⚠️ Could not fetch Telegram credentials from Supabase');
    }
  } catch (err) {
    console.warn('⚠️ Error loading Telegram credentials:', err.message);
  }
