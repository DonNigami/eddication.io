...
class SheetActions {
  ...
  async search(keyword, userId) {
    try {
      // Read jobdata sheet
      const jobs = await this.db.readRange(SHEETS.JOBDATA, 'A:AZ');
      ...
      for (let i = 1; i < jobs.length; i++) {
        if (jobs[i][referenceIdx] && 
            String(jobs[i][referenceIdx]).toUpperCase() === String(keyword).toUpperCase()) {
          matchingStops.push(this._rowToObject(headers, jobs[i]));
        }
      }
