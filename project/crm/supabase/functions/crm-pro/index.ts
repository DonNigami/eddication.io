...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === 'update-points') {
        ...
        const { error: logError } = await supabase
            ...
            .insert({
                ...
                points_before: user.points || 0,
                points_after: newPoints,
                action: 'manual_adjustment',
                reason: body.reason || 'Admin adjustment',
                created_by: body.adminId || 'system',
    ...
    if (action === 'broadcast') {
        ...
        // Log audit trail for broadcast
        const { error: auditError } = await supabase
            .from('audit_logs')
            .insert({
                action: 'broadcast',
                actor_id: body.adminId || 'system',
                target_type: target,
                ...
                metadata: {
                    message_preview: message?.substring(0, 100) || 'N/A',
                    target: target,
                    msgType: msgType,
                    requestId: requestId
                },
