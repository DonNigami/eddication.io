...
-- Policy: Admin users can view all transactions (check if role column exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'is_admin') THEN
        EXECUTE 'CREATE POLICY "Admin can view all transactions" 
    ...
    ELSIF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'role') THEN
        EXECUTE 'CREATE POLICY "Admin can view all transactions" 
        ON transaction_history FOR SELECT 
        USING (
            EXISTS (
    ...
    ELSE
        -- Fallback: Service role can view all
        EXECUTE 'CREATE POLICY "Admin can view all transactions" 
        ON transaction_history FOR SELECT 
...
-- Policy: Only admins can view audit logs (check if role column exists)
DO $$ 
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'profiles' AND column_name = 'is_admin') THEN
        EXECUTE 'CREATE POLICY "Admin can view audit logs" 
