...
const Settings = {
   ...
   */
  loadSettings() {
    chrome.storage.local.get(['geminiApiKey', 'openaiApiKey', 'selectedModel', 'downloadDelay', 'showDebugButtons', 'skipDownload'], (result) => {
      ...
      // Load download delay (default 15 if skipDownload is enabled, otherwise 90)
      this.downloadDelay = result.downloadDelay || (this.skipDownload ? 15 : 90);
  ...
  saveSettings() {
    ...
    let downloadDelay = parseInt(this.downloadDelaySelect?.value || '90', 10);
    if (skipDownload && downloadDelay === 90) {
      downloadDelay = 15;
      if (this.downloadDelaySelect) {
    ...
    chrome.storage.local.set({ geminiApiKey, openaiApiKey, selectedModel, downloadDelay, showDebugButtons, skipDownload }, () => {
      this.updateDebugButtonsVisibility();
