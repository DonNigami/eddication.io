...
async function init() {
  ...
  // Fix built-in prompts with broken thumbnail paths (one-time)
  const fixKey = 'promptThumbnailsFixedV2';
  const { [fixKey]: alreadyFixed } = await chrome.storage.local.get(fixKey);

  if (!alreadyFixed) {
    console.log('Fixing built-in prompt thumbnails...');
    // Delete all built-in prompts and let user re-import
    await PromptStorage.resetBuiltInPrompts();
    await chrome.storage.local.set({ [fixKey]: true });
    console.log('Built-in prompts reset. Please click "นำเข้า Default" to re-import.');
  }

  // Fix AI Story thumbnails ที่ว่างเปล่า (V3)
  const fixKeyV3 = 'aiStoryThumbnailsFixedV3';
  const { [fixKeyV3]: alreadyFixedV3 } = await chrome.storage.local.get(fixKeyV3);

  if (!alreadyFixedV3) {
    console.log('Fixing AI Story thumbnails...');
    const result = await PromptStorage.reimportAIStoryDefaults();
    await chrome.storage.local.set({ [fixKeyV3]: true });
    console.log('AI Story thumbnails fixed:', result.fixed, 'templates updated');
  }

  // V4: Force update AI Story systemPrompts - ห้ามบทพูด/สัดส่วนภาพใน image prompts
  const fixKeyV4 = 'aiStorySystemPromptsV4';
  const { [fixKeyV4]: alreadyFixedV4 } = await chrome.storage.local.get(fixKeyV4);

  if (!alreadyFixedV4) {
    console.log('Updating AI Story system prompts (V4)...');
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV4]: true });
    console.log('AI Story system prompts updated:', result.updated, 'templates');
  }

  // V5: Image prompts ต้องเน้นบรรยายฉากภาพนิ่งเท่านั้น (ไม่มีชื่อ/อารมณ์/การเคลื่อนไหว)
  const fixKeyV5 = 'aiStoryImageOnlyV5';
  const { [fixKeyV5]: alreadyFixedV5 } = await chrome.storage.local.get(fixKeyV5);

  if (!alreadyFixedV5) {
    console.log('Updating AI Story image prompts (V5) - static scene description only...');
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV5]: true });
    console.log('AI Story image prompts updated:', result.updated, 'templates');
  }

  // V6: MV/Music templates ให้รองรับการวิเคราะห์เนื้อเพลงและสร้างฉาก MV ที่เหมาะสม
  const fixKeyV6 = 'aiStoryMusicMVV6';
  const { [fixKeyV6]: alreadyFixedV6 } = await chrome.storage.local.get(fixKeyV6);

  if (!alreadyFixedV6) {
    console.log('Updating MV/Music templates (V6) - lyrics analysis support...');
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV6]: true });
    console.log('MV/Music templates updated:', result.updated, 'templates');
  }

  // V7: MV templates - ห้าม copy เนื้อเพลง ต้องแปลงเป็น prompt
  const fixKeyV7 = 'aiStoryMusicNoCopyV7';
  const { [fixKeyV7]: alreadyFixedV7 } = await chrome.storage.local.get(fixKeyV7);

  if (!alreadyFixedV7) {
    console.log('Updating MV templates (V7) - no copy lyrics, must transform...');
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV7]: true });
    console.log('MV templates updated:', result.updated, 'templates');
  }

  // V8: MV templates - output ต้องเป็น prompt ล้วนๆ ไม่มีหัวข้อ/ฉาก/เนื้อเพลง
  const fixKeyV8 = 'aiStoryMusicPurePromptV8';
  const { [fixKeyV8]: alreadyFixedV8 } = await chrome.storage.local.get(fixKeyV8);

  if (!alreadyFixedV8) {
    console.log('Updating MV templates (V8) - pure prompt output only...');
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV8]: true });
    console.log('MV templates updated:', result.updated, 'templates');
  }

  // V9: Video templates (horror, action, lofi) - output ต้องเป็น prompt ล้วนๆ
  const fixKeyV9 = 'aiStoryVideoPurePromptV9';
  const { [fixKeyV9]: alreadyFixedV9 } = await chrome.storage.local.get(fixKeyV9);

  if (!alreadyFixedV9) {
    console.log('Updating video templates (V9) - pure prompt output only...');
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV9]: true });
    console.log('Video templates updated:', result.updated, 'templates');
  }

  // V10: ลบ video templates เก่า (drama, horror, action, romantic, funny) แทนที่ด้วย สัตว์น่ารัก
  const fixKeyV10 = 'aiStoryCuteAnimalsV10';
  const { [fixKeyV10]: alreadyFixedV10 } = await chrome.storage.local.get(fixKeyV10);

  if (!alreadyFixedV10) {
    console.log('Replacing video templates (V10) - cute animals...');
    // ลบ templates เก่า
    const oldVideoTemplateIds = [
      'story-video-funny', 'story-video-drama', 'story-video-romantic',
      'story-video-horror', 'story-video-action'
    ];
    for (const id of oldVideoTemplateIds) {
      try {
    ...
    // Import templates ใหม่
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV10]: true });
    console.log('Cute animal video templates added:', result.updated, 'templates');
  }

  // V11: ลบ image templates เก่า (funny-clip, drama, romantic, horror, action) แทนที่ด้วย สัตว์น่ารัก
  const fixKeyV11 = 'aiStoryCuteAnimalsImageV11';
  const { [fixKeyV11]: alreadyFixedV11 } = await chrome.storage.local.get(fixKeyV11);

  if (!alreadyFixedV11) {
    console.log('Replacing image templates (V11) - cute animals...');
    // ลบ templates เก่า
    const oldImageTemplateIds = [
      'story-funny-clip', 'story-drama', 'story-romantic',
      'story-horror', 'story-action'
    ];
    for (const id of oldImageTemplateIds) {
      try {
        await PromptStorage.delete(id);
      } catch (e) {
        // ignore if not exists
    ...
    // Import templates ใหม่
    const result = await PromptStorage.forceUpdateAIStoryTemplates();
    await chrome.storage.local.set({ [fixKeyV11]: true });
    console.log('Cute animal image templates added:', result.updated, 'templates');
  }

  // V12: Video templates - เพิ่มคำสั่ง "ตอบเป็น prompt เดียว" ในทุก template
  const fixKeyV12 = 'videoSinglePromptV12';
  const { [fixKeyV12]: alreadyFixedV12 } = await chrome.storage.local.get(fixKeyV12);

  if (!alreadyFixedV12) {
    console.log('Updating video templates (V12) - single prompt only constraint...');
    const result = await PromptStorage.forceUpdateVideoTemplates();
    await chrome.storage.local.set({ [fixKeyV12]: true });
    console.log('Video templates updated:', result.updated, 'templates');
  }
