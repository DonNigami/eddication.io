...
const TikTokUploader = {
  ...
  setDefaultScheduleTime() {
    ...
    // Format to local datetime string (YYYY-MM-DDTHH:MM) for Bangkok timezone
...
ตอบแค่แคปชั่นบรรทัดเดียว ไม่ต้องอธิบาย`;
  ...
  async runAutomation() {
    ...
    // Check if current tab is TikTok upload page (support both URLs with or without query string)
    const isTikTokUploadPage = tab && tab.url && (
      /tiktok\.com\/tiktokstudio\/upload/.test(tab.url) ||
      /tiktok\.com\/creator-center\/upload/.test(tab.url)
    );

    console.log('[TikTok Automation] Current URL:', tab?.url);
    console.log('[TikTok Automation] Is TikTok upload page:', isTikTokUploadPage);

    if (!isTikTokUploadPage) {
      ...
      if (tiktokTabs.length > 0) {
        ...
        console.log('[TikTok Automation] Found existing TikTok tab:', tiktokTabs[0].url);
        tab = tiktokTabs[0];
        ...
        console.log('[TikTok Automation] Creating new TikTok tab');
        ...
        await new Promise((resolve) => {
          chrome.tabs.onUpdated.addListener(function listener(tabId, changeInfo) {
            if (tabId === tab.id && changeInfo.status === 'complete') {
              console.log('[TikTok Automation] New tab loaded:', changeInfo.status);
      ...
      // Already on TikTok upload page, wait for stability
      console.log('[TikTok Automation] Already on TikTok upload page');

      // Remove query parameters if any (e.g., ?from=creator_center causes WebSocket issues)
      if (tab.url.includes('?')) {
        const baseUrl = tab.url.split('?')[0];
        console.log('[TikTok Automation] Removing query parameters, navigating to:', baseUrl);
    ...
    // Inject content script to ensure it's loaded (for both existing and new tabs)
    try {
      console.log('[TikTok Automation] Injecting content script to tab:', tab.id);
      await chrome.scripting.executeScript({
        target: { tabId: tab.id },
        files: ['content.js']
      });
      console.log('[TikTok Automation] Content script injected successfully');
      await new Promise(resolve => setTimeout(resolve, 2000));
    } catch (e) {
      console.log('[TikTok Automation] Content script injection error:', e.message);
      // Try to proceed anyway as script might already be loaded
      await new Promise(resolve => setTimeout(resolve, 2000));
    }
