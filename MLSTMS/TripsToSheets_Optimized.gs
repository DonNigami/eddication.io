...
// ============================================
// PHASE 2 OPTIMIZATION HELPERS
// ============================================

/**
 * สร้าง Map<TripID, {status_id, updated_at}> จาก TripDetails Sheet
 * @returns {Map<string, {status_id: string, updated_at: string}>}
 */
function buildExistingTripInfoMap() {
  const config = getConfig();
  const ss = SpreadsheetApp.getActiveSpreadsheet();
  const sheet = ss.getSheetByName(config.tripDetailsSheetName);

  if (!sheet) {
    Logger.log("⚠️ buildExistingTripInfoMap: Sheet not found");
    return new Map();
  }

  const lastRow = sheet.getLastRow();
  if (lastRow <= 1) {
    Logger.log("⚠️ buildExistingTripInfoMap: No data in sheet");
    return new Map(); // No data
  }

  // Column indices: Trip ID = 1, Status ID = 5, Updated At = 12
  const data = sheet.getRange(2, 1, lastRow - 1, 12).getValues();
  const map = new Map();

  for (const row of data) {
    const tripId = String(row[0] || "").trim(); // Trip ID
    const statusId = String(row[4] || "").trim(); // Status ID column
    const updatedAt = row[11]; // Updated At column
  ...
  Logger.log(`✅ buildExistingTripInfoMap: Built map with ${map.size} entries`);
  return map;
}

/**
 * ตรวจสอบว่า status เป็นสถานะสุดท้ายหรือไม่
 * @param {string} statusId - Status ID
 * @returns {boolean} - true ถ้าเป็นสถานะสุดท้าย (1=Finished, 5=Timeout)
 */
function isFinalStatus(statusId) {
  const finalStatuses = ["1", "5"]; // Finished, Timeout
  return finalStatuses.includes(String(statusId || "").trim());
}

/**
 * ตรวจสอบว่า status เป็นสถานะ active หรือไม่
 * @param {string} statusId - Status ID
 * @returns {boolean} - true ถ้าเป็นสถานะ active (2=Wasting, 3=Pending, 4=Working)
 */
function isActiveStatus(statusId) {
  const activeStatuses = ["2", "3", "4"]; // Wasting, Pending, Working
  return activeStatuses.includes(String(statusId || "").trim());
}

/**
 * แปลง Bangkok time string → Date object (สำหรับเปรียบเทียบ)
 * @param {string} bangkokTime - "dd/MM/yyyy HH:mm:ss"
 * @returns {Date|null} - Date object หรือ null ถ้า parse ไม่สำเร็จ
 */
function bangkokTimeToDate(bangkokTime) {
  if (!bangkokTime) return null;

  try {
    // Parse dd/MM/yyyy HH:mm:ss
    const parts = bangkokTime.split(/[\s\/:]/);
    ...
    Logger.log(`⚠️ bangkokTimeToDate failed: ${bangkokTime} - ${error.message}`);
  }
...
function pullTripIdsPhase1(startDate, endDate, statusId = "ALL") {
  ...
  try {
    ...
    // ✅ เก็บ Trip IDs + updated_at + status_id ไว้สำหรับ Phase 2
    const tripInfoArray = [];
    const statusSummary = {}; // ✅ เก็บสรุปแยกตาม status

    for (const trip of allTripsForAllStatuses) {
      const tripId = getTripField(trip, ["id", "tripId", "trip_code", "tripCode", "trip_id"]);
      const updatedAt = getTripField(trip, ["updatedAt", "updated_at", "updatedDate"]);
      const statusId = getTripField(trip, [
        "tripStatus.statusId",
        "statusId",
        "status_id",
        "tripStatus.id"
      ...
      if (tripId) {
        ...
        if (!statusSummary[normalizedStatusId]) {
          statusSummary[normalizedStatusId] = { fetched: 0, saved: 0 };
        }
        statusSummary[normalizedStatusId].fetched++;
      }
    }

    const latestTripId = tripInfoArray.length > 0 ? tripInfoArray[tripInfoArray.length - 1].id : "";

    scriptProperties.setProperty("LATEST_TRIP_ID", latestTripId);
    scriptProperties.setProperty("TOTAL_TRIP_IDS", tripInfoArray.length.toString());
    scriptProperties.setProperty("TRIP_INFOS", JSON.stringify(tripInfoArray));
    ...
    const statusNames = {
      ...
      "2": "Wasting (เลยกำหนด)",
      "3": "Pending (รอเวลา)",
      "4": "Working (กำลังขนส่ง)",
      "5": "Timeout (ปิดเลยเวลา)"
    };

    let statusBreakdown = "";
    for (const statusId of Object.keys(statusSummary).sort()) {
      const count = statusSummary[statusId].fetched;
      statusBreakdown += `• Status ${statusId} (${statusNames[statusId]}): ${count} รายการ\n`;
    }
    ...
    const summaryMessage =
      ...
      `• เพิ่มใหม่: ${tripInfoArray.length - actualTotalTrips > 0 ? (tripInfoArray.length - actualTotalTrips) : 0} รายการ\n` +
      `• อัพเดท: ${actualTotalTrips} รายการ\n\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `📊 แยกตาม Status:\n` +
      statusBreakdown +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n` +
    ...
    // ✅ บันทึก Execution Log แบบละเอียด
    logExecution(
      "pullTripIdsPhase1",
      true,
      `Phase 1 Complete`,
      {
        totalTrips: totalFetched,
        fetched: totalFetched,
        saved: actualTotalTrips,
        timeTaken: `${Math.floor((Date.now() - phase1StartTime) / 60000)}m ${Math.floor(((Date.now() - phase1StartTime) % 60000) / 1000)}s`,
        statusBreakdown: Object.keys(statusSummary).sort().map(sid => {
          const statusNames = {
            "1": "Finished",
            "2": "Wasting",
            "3": "Pending",
          ...
          return `Status ${sid} (${statusNames[sid]}): ${statusSummary[sid].fetched} trips`;
        }).join(" | ")
      }
    );
...
function pullTripDetailsPhase2() {
  ...
  try {
    // ✅ อ่าน Trip Infos (id + updated_at + status_id) จาก Phase 1
    const tripInfos = readTripInfosFromPhase1();

    if (tripInfos.length === 0) {
      ui.alert(
        "⚠️ No Trip Infos Found",
        `ไม่พบ Trip Infos จาก Phase 1\n\n` +
    ...
    const confirmed = ui.alert(
      "📋 Phase 2: ดึง Trip Details (Optimized)",
      `จำนวน Trip IDs จาก Phase 1: ${tripInfos.length} รายการ\n\n` +
        `✅ Smart Filtering enabled:\n` +
        `• Status 1,5 (Finished/Timeout) → Skip ถ้ามีในชีท\n` +
        `• Status 2,3,4 (Active) → ดึงเสมอ\n` +
        `• Updated At เปรียบเทียบ → Skip ถ้าไม่เปลี่ยน\n\n` +
        `เวลาโดยประมาณ: ~${Math.ceil((tripInfos.length * 0.3) / 60)} นาที (ลด 50%+)\n\n` +
    ...
    // ✅ สร้าง Map ของ {status_id, updated_at} จาก TripDetails Sheet
    const existingTripInfoMap = buildExistingTripInfoMap();
    Logger.log(`📊 Phase 2: พบ trips ที่มีอยู่ในชีท ${existingTripInfoMap.size} รายการ`);

    // ✅ Smart Filter: เลือกเฉพาะ trips ที่ต้องดึง
    const tripsToFetch = [];
    const tripsToSkip = [];
    const skipReasons = {
      finalStatus: 0,    // Skip เพราะ status 1,5 (สถานะสุดท้าย)
      noChange: 0,       // Skip เพราะ updated_at เท่ากัน
      newTrip: 0         // New trips
    };

    for (const info of tripInfos) {
      const tripId = info.id;
      const apiStatusId = String(info.status_id || "").trim();
      const apiUpdatedAt = info.updated_at || "";

      const existingInfo = existingTripInfoMap.get(tripId);
      const sheetStatusId = existingInfo ? existingInfo.status_id : "";
      const sheetUpdatedAt = existingInfo ? existingInfo.updated_at : "";

      // ถ้าไม่มีในชีท → ต้องดึง
      if (!existingInfo) {
      ...
      // ถ้า status เป็น 1 (Finished) หรือ 5 (Timeout) และมีในชีทแล้ว → SKIP
      if (isFinalStatus(apiStatusId)) {
        tripsToSkip.push(tripId);
        skipReasons.finalStatus++;
        continue;
      }

      // ถ้า status เป็น active (2, 3, 4) → ต้องดึงเสมอ
      if (isActiveStatus(apiStatusId)) {
        tripsToFetch.push(tripId);
        continue;
      }

      // ถ้า status เปลี่ยน (เช่น 3 → 1) → ดึง
      if (apiStatusId !== sheetStatusId) {
        tripsToFetch.push(tripId);
        continue;
      }

      // เปรียบเทียบ updated_at
      const apiUpdatedAtBangkok = apiUpdatedAt ? formatISOToBangkok(apiUpdatedAt) : "";

      // ถ้า API ใหม่กว่า → ต้องดึง
      if (!sheetUpdatedAt || (apiUpdatedAtBangkok && apiUpdatedAtBangkok > sheetUpdatedAt)) {
        tripsToFetch.push(tripId);
      } else {
        // updated_at เท่ากัน → skip
        tripsToSkip.push(tripId);
        skipReasons.noChange++;
      }
    }

    Logger.log(`✅ Phase 2: จะดึง ${tripsToFetch.length} รายการ, skip ${tripsToSkip.length} รายการ`);
    Logger.log(`📊 Skip reasons:`);
    Logger.log(`   - Final status (1,5): ${skipReasons.finalStatus}`);
    Logger.log(`   - No change: ${skipReasons.noChange}`);
    Logger.log(`   - New trips: ${skipReasons.newTrip}`);

    ss.toast(
      `📊 ดึง ${tripsToFetch.length}, Skip ${tripsToSkip.length} (สถานะสุดท้าย: ${skipReasons.finalStatus})`,
      "MLSTMS Trips",
      10
    );
    ...
    ss.toast(`📋 Phase 2: กำลังดึง Trip Details ${tripsToFetch.length} รายการ...`, "MLSTMS Trips", 10);
    Logger.log(`📋 Phase 2: Starting to fetch ${tripsToFetch.length} trip details...`);

    for (let i = 0; i < tripsToFetch.length; i++) {
      const tripId = tripsToFetch[i];
      const progress = Math.round(((i + 1) / tripsToFetch.length) * 100);

      // Progress update ทุก 10% หรือทุก 50 รายการ
      if ((i + 1) % 50 === 0 || (i + 1) % Math.floor(tripsToFetch.length / 10) === 0 || i === tripsToFetch.length - 1) {
        ss.toast(
          `📋 กำลังดึง Details: ${i + 1}/${tripsToFetch.length} (${progress}%)`,
          "MLSTMS Trips",
          5,
        );
        Logger.log(`   📊 Progress: ${i + 1}/${tripsToFetch.length} (${progress}%) - fetched: ${successful}, failed: ${failed}`);
      }
    ...
    Logger.log(`   ✅ Skipped ${tripsToSkip.length} trips (ไม่มีการเปลี่ยนแปลง)`);

    // ✅ สรุปแยกตาม status สำหรับ trips ที่ดึง
    const statusFetchSummary = {};
    const statusSkipSummary = {};

    // นับจำนวน trips ที่ดึงแยกตาม status
    for (const info of tripInfos) {
      const statusId = String(info.status_id || "").trim();
      if (tripsToFetch.includes(info.id)) {
        // Trip นี้ถูกดึง
        if (!statusFetchSummary[statusId]) statusFetchSummary[statusId] = 0;
        statusFetchSummary[statusId]++;
      } else {
        // Trip นี้ถูก skip
        if (!statusSkipSummary[statusId]) statusSkipSummary[statusId] = 0;
        statusSkipSummary[statusId]++;
      }
    }
    ...
    // ✅ สร้างรายงละเอียด status breakdown
    const statusNames = {
      "1": "Finished",
      "2": "Wasting",
      "3": "Pending",
    ...
    let fetchStatusBreakdown = "";
    for (const statusId of Object.keys(statusFetchSummary).sort()) {
      const count = statusFetchSummary[statusId];
      fetchStatusBreakdown += `  • Status ${statusId} (${statusNames[statusId]}): ${count} รายการ\n`;
    }

    let skipStatusBreakdown = "";
    for (const statusId of Object.keys(statusSkipSummary).sort()) {
      const count = statusSkipSummary[statusId];
      skipStatusBreakdown += `  • Status ${statusId} (${statusNames[statusId]}): ${count} รายการ\n`;
    }

    const summaryMessage =
      `✅ Phase 2 Complete! (Optimized)\n\n` +
      `📊 สรุปเสร็จ:\n` +
      `• ดึงข้อมูลสำเร็จ: ${successful} รายการ\n` +
      `• ล้มเหลว: ${failed} รายการ\n` +
      `• บันทึกลงชีท: ${allDetails.length} รายการ\n` +
      `• ⚡ Skip ที่ไม่ต้องดึง: ${tripsToSkip.length} รายการ (${Math.round(tripsToSkip.length / (tripsToFetch.length + tripsToSkip.length) * 100)}%)\n\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `📊 ดึงข้อมูล (Fetch) - แยกตาม Status:\n` +
      (fetchStatusBreakdown || "  ไม่มีข้อมูล\n") +
      `\n` +
      `📊 Skip ไม่ดึง - แยกตาม Status:\n` +
      (skipStatusBreakdown || "  ไม่มีข้อมูล\n") +
      `\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `📊 Skip Reasons (รวม):\n` +
      `• สถานะสุดท้าย (1,5): ${skipReasons.finalStatus}\n` +
      `• ไม่มีการเปลี่ยนแปลง: ${skipReasons.noChange}\n` +
      `• New trips: ${skipReasons.newTrip}\n\n` +
      `━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n` +
      `⏱️ เวลาที่ใช้: ${minutes} นาที ${seconds} วินาที\n` +
      `💾 ประหยัด API calls: ${tripsToSkip.length} ครั้ง (${Math.round(tripsToSkip.length / (tripsToFetch.length + tripsToSkip.length) * 100)}%)`;

    ui.alert("✅ Phase 2 Complete!", summaryMessage, ui.ButtonSet.OK);
    Logger.log(`✅ Phase 2 Complete: ${allDetails.length} details fetched, ${tripsToSkip.length} skipped (${Math.round(tripsToSkip.length / (tripsToFetch.length + tripsToSkip.length) * 100)}% saved) in ${minutes}m ${seconds}s`);

    // ✅ บันทึก Execution Log แบบละเอียด
    logExecution(
      "pullTripDetailsPhase2",
      true,
      `Phase 2 Complete (Optimized) - Saved ${allDetails.length} trips`,
      {
        totalTrips: tripInfos.length,
        fetched: successful,
        saved: allDetails.length,
        timeTaken: `${minutes}m ${seconds}s`,
        statusBreakdown: Object.keys(statusFetchSummary).sort().map(sid => {
          const statusNames = {
            "1": "Finished",
            "2": "Wasting",
            "3": "Pending",
          ...
          const fetchCount = statusFetchSummary[sid] || 0;
          const skipCount = statusSkipSummary[sid] || 0;
          return `Status ${sid}: ${fetchCount} fetched, ${skipCount} skipped`;
        }).join(" | ")
      }
    );
  } catch (error) {
...
/**
 * อ่าน Trip Infos (id + updated_at + status_id) จาก Phase 1 Storage
 * ✅ ใช้สำหรับ Phase 2 Optimization
 * @returns {Array<{id: string, updated_at: string, status_id: string}>}
 */
function readTripInfosFromPhase1() {
  const scriptProperties = PropertiesService.getScriptProperties();
  const tripInfosJson = scriptProperties.getProperty("TRIP_INFOS");

  if (!tripInfosJson) {
    Logger.log("⚠️ readTripInfosFromPhase1: No TRIP_INFOS found");
    return [];
  }

  try {
    const tripInfos = JSON.parse(tripInfosJson);
    return Array.isArray(tripInfos) ? tripInfos : [];
  } catch (error) {
    Logger.log(
      `⚠️ Error reading Trip Infos from Phase 1 storage: ${error.message}`,
    );
    return [];
  }
}
...
function viewLatestExecutionLog() {
  const ui = SpreadsheetApp.getUi();
  const logs = SpreadsheetApp.getActiveSpreadsheet().getSheetByName("Execution Logs");
  ...
  // ✅ อ่าน headers เพื่อตรวจสอบ column ที่มี
  const headers = logs.getRange(1, 1, 1, logs.getLastColumn()).getValues()[0];
  const hasDetailedColumns = headers.length >= 9;

  // อ่าน log ล่าสุด 5 รายการ
  const lastRow = logs.getLastRow();
  const rowsToRead = Math.min(5, lastRow - 1);
  const logData = logs.getRange(lastRow - rowsToRead + 1, 1, rowsToRead, hasDetailedColumns ? 9 : 4).getValues();
  ...
  for (let i = logData.length - 1; i >= 0; i--) {
    ...
    // ✅ แสดงข้อมูลละเอียดถ้ามี
    if (hasDetailedColumns) {
      const totalTrips = row[3] || "";
      const fetched = row[4] || "";
      const saved = row[5] || "";
      const timeTaken = row[6] || "";
      const statusBreakdown = row[7] || "";
      const details = row[8] || "";

      if (totalTrips) message += `รวม Trips: ${totalTrips}\n`;
      if (fetched) message += `ดึงข้อมูล: ${fetched}\n`;
      if (saved) message += `บันทึก: ${saved}\n`;
      if (timeTaken) message += `เวลา: ${timeTaken}\n`;
      if (statusBreakdown) message += `สถานะ:\n  ${statusBreakdown.replace(/ \| /g, "\n  ")}\n`;
      if (details) message += `รายละเอียด: ${details}\n`;
    } else {
 ...
 * ✅ บันทึกต่อท้ายล่าสุด (แทนที่จะต่อท้ายทั้งหมด)
 *
 * @param {string} functionName - ชื่อฟังก์ชันที่รัน
 * @param {boolean} success - สถานะความสำเร็จ
 * @param {string} details - รายละเอียด
 * @param {Object} metadata - ข้อมูลเพิ่มเติม (statusSummary, saveSummary, etc.)
 */
function logExecution(functionName, success, details, metadata = {}) {
  try {
    const ss = SpreadsheetApp.getActiveSpreadsheet();
    ...
    if (!logSheet) {
      ...
      logSheet.setColumnWidth(2, 200); // Function
      logSheet.setColumnWidth(3, 100); // Status
      logSheet.setColumnWidth(4, 500); // Details (wider for detailed info)
    }

    // ✅ ตรวจสอบว่ามี header แล้วหรือยัง
    const hasHeaders = logSheet.getLastRow() > 0;
    if (!hasHeaders) {
      Logger.log("⚠️ logExecution: No headers found, creating...");
      return; // ไม่ควรเกิดขึ้นมา ถ้าไม่มี header
    }

    const timestamp = new Date();
    const status = success ? "✅ Success" : "❌ Failed";

    // ✅ Build detailed message combining all metadata
    let detailMessage = details;

    // Add metadata to details if available
    if (metadata.totalTrips !== undefined) {
      detailMessage += `\n📊 Total Trips: ${metadata.totalTrips}`;
    }
    if (metadata.fetched !== undefined) {
      detailMessage += `\n📥 Fetched: ${metadata.fetched}`;
    }
    if (metadata.saved !== undefined) {
      detailMessage += `\n💾 Saved: ${metadata.saved}`;
    }
    if (metadata.timeTaken !== undefined) {
      detailMessage += `\n⏱️ Time: ${metadata.timeTaken}`;
    }
    if (metadata.statusBreakdown) {
      detailMessage += `\n📈 Status Breakdown:\n${metadata.statusBreakdown}`;
    }

    // ✅ Format details ให้อ่านง่าย
    const shortDetails = detailMessage.length > 1000
      ? detailMessage.substring(0, 1000) + "..."
      : detailMessage;

    // ✅ บันทึกที่บรรทัด 2 (ต่อจาก header)
    const targetRow = 2; // Row 2 = แถวแรกถัดไปหลัง header

    // ✅ Shift rows ลงมาเพื่อเว้ที่บรรทัด 2
    const lastRow = logSheet.getLastRow();
    if (lastRow >= targetRow) {
      // มีข้อมูลอยู่แล้ว → insert new row at position 2, pushing existing data down
      logSheet.insertRowBefore(targetRow);
    }

    // ✅ เขียนข้อมูล log ใหม่ที่บรรทัด 2 (4 columns only)
    logSheet.getRange(targetRow, 1, 1, 4).setValues([[
      timestamp,
      functionName,
      status,
      shortDetails
    ...
    Logger.log(`📋 Execution Logged at row ${targetRow}: ${functionName} - ${status}`);
  } catch (error) {
